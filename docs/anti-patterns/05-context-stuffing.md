...
あるチームがRAGベースの社内ドキュメント検索エージェントを構築しました。「情報が足りないよりは多い方がよい」という判断で、検索の top-k を20に設定し、取得した全チャンクをプロンプトに投入しました。初期テストでは回答品質が良好でしたが、ドキュメントが増えるにつれ問題が表面化しました。関連度の低いチャンクがノイズとなり、エージェントが無関係な情報を引用して誤った回答を生成するケースが増えました。

さらに、1リクエストあたりのトークン数が平均8,000から30,000に膨らみ、コストが4倍近くに増大しました。レイテンシも悪化し、応答に5秒以上かかるようになりました。
...
「コンテキストに情報を多く入れれば、モデルが適切に取捨選択してくれる」という期待が根底にあります。しかし、研究によればLLMは長いコンテキストの中間部分を見落としやすく（Lost in the Middle問題）、ノイズが多いほど回答品質は低下します。

top-k の値やチャンクサイズの設計を `[F7]` コスト感度や `[F4]` レイテンシ予算と結びつけて検討していないことが原因です。「検索で取れるだけ取って全部入れる」というアプローチは、情報の選別コストをLLMに転嫁しているに過ぎません。
...
- [#24 Context Pack / Assembly](../patterns/05-memory-context/24-context-pack-assembly.md) — 関連度の高い情報のみを選択的に組み立てます
- [#23 Layered Memory](../patterns/05-memory-context/23-layered-memory.md) — メモリを階層化してアクセスパターンを最適化します
- [#25 Memory Write Gate](../patterns/05-memory-context/25-memory-write-gate.md) — 記憶する情報自体を選別します
