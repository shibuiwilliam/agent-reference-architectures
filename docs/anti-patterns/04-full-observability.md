...
あるチームが本番エージェントの品質監視のために、全リクエストのプロンプトと応答をリアルタイムで分析パイプラインに流す構成を組みました。各リクエストに対して、毒性スコア、幻覚検出、トピック分類、感情分析をLLMベースの評価器で実行していました。初期は数百リクエスト/日で問題なかったのですが、利用者が増えて数万リクエスト/日になった頃、観測パイプラインのLLMコストがエージェント本体の3倍に達してしまいました。

ストレージも膨れ上がりました。全トレースを無期限で保持していたため数ヶ月で数TBに達し、障害調査のためのログ検索に数分かかる状態です。「観測のための観測」が本来の目的を妨げるという皮肉な状況に陥りました。
...
「何が起きるか分からないから全部記録しておこう」という不安駆動の設計が原因です。従来のWebシステムではログのコストは相対的に低いものでしたが、LLMベースの評価器を使った品質分析は1件あたりのコストがかなり高くつきます。加えて、ログの保持期間やサンプリング率を段階的に設計するノウハウもまだ十分に確立されていません。
