...
パターンは「問題に対する解」である。AIエージェントのアーキテクチャパターンを理解するには、まずAIエージェントが本番環境にどのような**新しい壊れ方**を持ち込むかを把握する必要がある。
...
各特性がどの程度のリスクになるかは、システムの文脈——すなわち [駆動変数（フォース）](forces.md) によって決まる。たとえば「非決定論的な出力」(C1) は、失敗コスト `[F2]` が高い医療・金融では致命的だが、レコメンドのように `[F2]` が低い場面では許容範囲になる。

パターンは特性に対する汎用的な防波堤だが、どのパターンをどの程度適用するかは駆動変数で決める。この判断を助けるのが [意思決定層](../decisions/tuning-dials.md) である。
