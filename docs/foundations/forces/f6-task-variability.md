...
請求書の定型処理なら手順をフローチャートに落とせるが、未知のバグを調査するタスクでは、どのファイルを読み、どのログを追い、誰に聞くかを実行しながら判断するしかない。この「手順を事前に決められるか」が、ワークフローで済むかエージェントが必要かの分岐点になる。

タスクの変動性は、処理手順の予測可能性を表す。このフォースは「LLMにどこまで判断を委ねるか」の根本的な分岐点を決める。
