...
社内の管理ツールとして使っていたエージェントを一般ユーザーにも公開したら、翌日にはプロンプトインジェクション攻撃で内部データが漏洩した——入力元が変わるだけで、システムに求められるセキュリティ対策はまったく異なる。

入力の信頼度は、エージェントが受け取るデータがどの程度信頼できるかを表す。社内システムからのAPI呼び出しと、不特定多数のユーザーからの自然言語入力では、リスクの水準が桁違いになる。
