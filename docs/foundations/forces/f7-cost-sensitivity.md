...
開発環境で快適に動いていたエージェントを本番に出した途端、月末のAPI請求書が想定の10倍に膨れ上がった——LLMの推論コストは従来のAPIコールと桁が違うため、トラフィック増加がそのまま予算超過に直結する。

コスト感度・スケールは、システムが処理するリクエスト量（QPS）と、月間で許容されるLLM API費用の上限の厳しさを表す。無制限に呼び出せば月額費用が急速に膨張するため、このフォースはコスト効率に関するすべての設計判断の起点となる。
