...
個別パターンは単独でも有用だが、本番システムでは複数のパターンを組み合わせて使う。以下に、典型的なユースケース別の複合構成を示す。

各構成は「このパターンを採用する理由」と「省略してよい条件」を併記する。全てを一度に導入する必要はない——最小構成から始め、[駆動変数](foundations/forces.md) の変化に応じて層を追加する。
...
構成は排他的ではない。副作用重視＋コスト重視のように、複数の構成を重ねることも多い。重要なのは、[駆動変数](foundations/forces.md) に基づいて「なぜこの層が必要か」を説明できることである。
