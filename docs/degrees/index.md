...
**原則：あらゆる目盛りは「効きすぎる害 ⇔ 効かなすぎる害」の中点を探す問題です。** 目安値はすべて出発点であり、[駆動変数](../concepts/driving-variables.md) で動かします。固定ハードコードは禁忌です。本番メトリクス（p99レイテンシ・ヒット率・評価スコア）から継続的に再調整する変数として扱ってください。
...
エラーは三分類で扱います。①一時障害（429/5xx/タイムアウト）→リトライ有効、②コンテンツ起因（スキーマ不適合・低品質）→単純リトライ無意味で self-correction へ、③コンテキスト長超過→要約・分割です。決済・発注など戻せない副作用は**自動リトライ禁止・状態確認後に補償**してください。

## 自律性・人間介在

自律性は有無ではなく段階で設計します（L0–L6）。固定せず実績で昇格/降格させます（[E5](../patterns/e-safety-hitl/e5-autonomy-ladder.md)）。
