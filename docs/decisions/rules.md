...
!!! abstract "一言"
    フォース評価から「まず検討すべきパターン候補」を絞り込む IF–THEN 規則。最終的には人間が判断する。

## この章の目的

[意思決定の進め方](decision-flow.md) のステップ②「ルール照合」で使う決定規則の一覧。`decisions.yml` の `rules[]` から自動生成される。

各規則は「フォースの組み合わせ → 必須/推奨/任意のパターン候補」を示す。あくまで**候補の提示**であり、断定ではない。実際のフォースの掛け合わせや制約の判断は、[通し例](worked-examples.md) で流れを追ってみてほしい。
