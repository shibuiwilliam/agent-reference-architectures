...
!!! abstract "一言"
    フォース評価から「まず検討すべきパターン候補」を絞り込む IF–THEN 規則です。最終的には人間が判断します。

## この章の目的

[意思決定の進め方](decision-flow.md) のステップ②「ルール照合」で使う決定規則の一覧です。`decisions.yml` の `rules[]` から自動生成しています。

各規則は「フォースの組み合わせ → 必須/推奨/任意のパターン候補」を示しています。あくまで**候補の提示**であり、断定ではありません。フォースの掛け合わせや制約をどう判断するかは、[通し例](worked-examples.md) で流れを追ってみてください。
...
1. [駆動変数（フォース）](../foundations/forces.md) を「高/中/低」で評価します
2. 上記の規則から、条件に合致するものを**すべて**収集します（複数ルールが同時に該当しえます）
3. **必須** パターンを採用候補に入れます
4. **推奨** パターンをフォースの組み合わせに応じて検討します
5. **任意** パターンは escalation 条件に該当する場合に昇格を検討します
6. [二者択一](tradeoffs.md) と [ダイヤル](tuning-dials.md) を併せて設計判断を完成させます

!!! warning "候補提示であり断定ではない"
    規則は「まず検討すべきパターン」を絞り込む手段であり、自動的に採用を決めるものではありません。最終判断は人間が行います。
