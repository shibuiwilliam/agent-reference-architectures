...
[意思決定の進め方](decision-flow.md) のステップ②「ルール照合」で使う決定規則の一覧です。`decisions.yml` の `rules[]` から自動生成されます。

各規則は「フォースの組み合わせ → 必須/推奨/任意のパターン候補」を示します。あくまで**候補の提示**であり、断定ではありません。実際のフォースの掛け合わせや制約の判断は、[通し例](worked-examples.md) で流れを追ってみてください。
