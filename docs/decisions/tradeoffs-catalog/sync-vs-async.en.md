...
[#58 Sync Facade over Async Core](../../decisions/tradeoffs-catalog/sync-vs-async.md) is the representative hybrid strategy. Internally, processing is always asynchronous, and if it completes within the threshold time, it is returned as a synchronous response. If it exceeds the threshold, it switches to a job ID. Clients enjoy the simplicity of synchronous for short tasks while ensuring long-running tasks are not interrupted.
...
- [#1 Request-to-Job Gateway](../../decisions/tradeoffs-catalog/sync-vs-async.md) — The basic implementation pattern for asynchronous request acceptance
- [#58 Sync Facade over Async Core](../../decisions/tradeoffs-catalog/sync-vs-async.md) — A hybrid that automatically switches between synchronous and asynchronous
