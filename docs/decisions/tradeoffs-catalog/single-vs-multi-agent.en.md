...
As with [#59 Spectrum Selector](../../patterns/01-execution/59-workflow-agent-spectrum-selector.md), there is an approach that dynamically selects single/multi on a per-subtask basis. At the routing or triage stage, difficulty is assessed, and simple tasks are handled by a single agent while only complex ones are routed to multi-agent.
...
- [#9 Supervisor & Specialist Agents](../../patterns/02-composition/09-supervisor-specialist-agents.md) — Multi-agent configuration with a supervisor and specialist roles
- [#59 Workflow-Agent Spectrum Selector](../../patterns/01-execution/59-workflow-agent-spectrum-selector.md) — A meta-pattern that selects single/multi per subtask
