...
As shown by [#40 Fallback & Graceful Degradation](../../decisions/tradeoffs-catalog/fail-fast-vs-degradation.md), set fail-fast timeouts at each level of the degradation staircase. Set time limits for each fallback destination, and ultimately fail-fast if all levels fail. The discipline of not attempting infinite degradation is important.
...
- [#40 Fallback & Graceful Degradation](../../decisions/tradeoffs-catalog/fail-fast-vs-degradation.md) — Design pattern for staged fallback
