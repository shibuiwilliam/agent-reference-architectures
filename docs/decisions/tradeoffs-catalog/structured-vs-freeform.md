...
エージェントの出力を次のAPIに渡したいのにJSON形式が崩れてパースエラーになる——構造化出力を強制すれば防げるが、ユーザー向けの丁寧な説明文までスキーマに押し込めると窮屈になる。出力の「消費者」が機械なのか人間なのかで、最適な制約レベルは変わる。

この二者択一は、出力をJSONスキーマ等で厳密に制約するか、自然言語のまま自由に生成させるかの選択である。パース可能性・型安全性と、表現の豊かさ・柔軟性のトレードオフになる。

## 選択肢の詳細

### 構造化出力の強制

出力をJSON Schema、Pydanticモデル、XML等で定義し、LLMにスキーマ準拠の出力を強制する。下流のコードが安全にパースでき、バリデーションも自動化できる。APIやワークフローとの連携で必須。ただし自然言語の微妙なニュアンスを構造に押し込めると情報が欠落する。スキーマの設計・保守コストも発生する。

### 自由形式出力

LLMが自然言語で自由に回答する。説明、要約、創作、対話など人間が直接読むユースケースに適する。出力の多様性や表現力が活きる。ただし出力のパースが困難で、下流処理との連携にはパーサーや後処理が必要。出力形式の一貫性も保証されない。
