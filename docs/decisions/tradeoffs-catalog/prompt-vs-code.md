...
「返金額が5万円を超えたら承認フローに回す」——このルールをプロンプトに自然言語で書くか、コードの `if` 文で書くかで、テスト可能性と変更速度が大きく変わる。プロンプトなら非エンジニアでも調整できるが、いつの間にかルールが変わっていても気づきにくい。

この二者択一は、エージェントの振る舞いをプロンプト（自然言語指示）で制御するか、コード（条件分岐・ルールエンジン・ポリシーファイル）で制御するかの選択である。変更の俊敏性と厳密性のトレードオフになる。

## 選択肢の詳細

### プロンプト制御

自然言語でエージェントの行動方針・判断基準・出力フォーマットを指示する。変更のデプロイが速く、非エンジニアでも調整可能。微妙なニュアンスの指示に向く。ただし挙動の再現性が低く、プロンプトの変更がどの出力に影響するかのテストが困難。バージョン間の差分も追いにくい。

### コード制御

条件分岐、バリデーション、ポリシールールをプログラムとして記述する。ユニットテスト・CI/CDの対象にでき、変更の影響範囲を静的解析で把握できる。監査証跡としてもGit履歴が機能する。反面、変更にはエンジニアが必要で、曖昧な判断の表現には不向き。
...
コード制御をデフォルトにする。テスト可能性とバージョン管理が初期段階から確保できる。プロンプトは「コードでは表現しにくい曖昧な判断」に限定して使う。
