...
エージェントがレポートを生成している間、ユーザーはブラウザの前で「今どこまで進んだのか」と待っている。進捗をリアルタイムにストリーミングすれば体験は良いが、常時接続の維持やロードバランサとの相性など、インフラ側の負担が増える。

この二者択一は、サーバーから能動的に通知するプッシュと、クライアントが定期的に問い合わせるプルの選択である。リアルタイム性とインフラ複雑性のトレードオフになる。
