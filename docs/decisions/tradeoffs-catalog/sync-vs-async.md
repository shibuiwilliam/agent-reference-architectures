...
エージェントにドキュメント要約を依頼したら10秒で返ってくることもあれば、3分かかることもある。この「処理時間が読めない」状況で、ユーザーにはどう返すべきだろうか。

この二者択一は、リクエストをHTTPレスポンスとして即座に返すか、ジョブIDを発行して後から結果を受け取るかの選択である。処理時間の予測可能性とUXの即応性のバランスが焦点になる。
