...
毎月の請求書処理のように手順が決まった業務と、ユーザーの曖昧な質問を調べて回答する調査業務では、最適な実行方式が根本的に異なる。前者はステップを固定した方が安定し、後者は状況に応じて動きを変える柔軟さが要る。

この二者択一は、処理フローを事前にコードで定義する決定論的ワークフローと、LLMが状況に応じて次のアクションを自律的に決めるエージェントの選択である。予測可能性と柔軟性のトレードオフになる。
