...
Use push as the primary mechanism while providing a polling endpoint as a fallback for when connections drop or Webhook delivery fails. [#7 Streaming Progress](../../decisions/tradeoffs-catalog/push-vs-pull.md) is the implementation pattern for push-based progress delivery, and it is common to co-locate a polling status API.
...
- [#7 Streaming Progress](../../decisions/tradeoffs-catalog/push-vs-pull.md) — Design pattern for push-based progress streaming
