...
この二者択一は、タスク全体を1つのエージェントに処理させるか、役割ごとに複数エージェントへ分担させるかの選択です。分割はコンテキスト窓の制約緩和や専門性の向上に効きますが、通信オーバーヘッドとデバッグ難度を引き上げます。

## 選択肢の詳細

### シングルエージェント

1つのエージェントがツール呼び出し・推論・出力を一貫して担います。コンテキストの受け渡しが不要でレイテンシも低く、プロンプトの管理が1箇所で済むのでテスト・デバッグも容易です。タスクの幅が狭く、コンテキスト窓に収まる場合はこれで十分です。
...
シングルで十分なら分割しないのが鉄則です。マルチエージェント化は、単一プロンプトでは品質が出ない、コンテキスト窓が溢れる、並列化で大幅に高速化できる、のいずれかが明確なときに検討します。
...
| # | パターン | 向き | 不向き |
|---|---------|------|--------|
| #9 | **Supervisor & Specialist Agents** | 多様なタスク種別（FAQ/技術/請求）、マルチモーダル、ドメイン専門性が分離可能 | タスク種別が少ない; 専門化の利益が薄い |
