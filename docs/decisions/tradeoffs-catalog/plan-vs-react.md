...
本番データベースのマイグレーションをエージェントに任せる場面を想像してほしい。いきなり手を動かし始めると取り返しがつかない。一方、社内Wikiの検索であれば、1ステップずつ結果を見ながら掘り進める方が効率的である。

この二者択一は、まず全体計画を立ててから実行するか、観察→思考→行動のサイクルを1ステップずつ回すかの選択である。計画の質と実行の適応性のバランスが問われる。
