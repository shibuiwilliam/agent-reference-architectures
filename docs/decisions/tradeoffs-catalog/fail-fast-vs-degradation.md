...
ECサイトの購買アシスタントがLLM障害でダウンしたとき、「現在利用できません」とエラーを返すか、キャッシュ済みのFAQ応答で凌ぐか。検索サジェストのような補助機能なら即座にエラーで構わないが、購買の途中で止まるとカート放棄につながる。

この二者択一は、障害時に即座にエラーを返して呼び出し元に判断を委ねるか、品質を段階的に落としながらも何らかの応答を返し続けるかの選択である。障害の波及範囲と応答継続性のトレードオフになる。
