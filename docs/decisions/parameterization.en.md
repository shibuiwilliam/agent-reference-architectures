...
[#2 Durable Agent Session](../decisions/tradeoffs-catalog/in-context-vs-external.md) is a pattern for persisting state, but "how frequently to take checkpoints" needs to be tuned as a parameter.
...
[#32 Agent Trace](../decisions/dials/trace-sampling-rate.md) is a pattern for logging every step, but recording 100% in production significantly inflates storage costs.
...
[#37 Semantic Gateway](../decisions/dials/model-tier-routing.md) is a pattern that routes to different models based on difficulty, and "at what threshold to route to the larger model" becomes the parameter.
...
5. **Record the rationale**: Document why each value was chosen in design documents or traces (→ [#32 Agent Trace](../decisions/dials/trace-sampling-rate.md))
