...
59個ものパターンを前にして「どれを使えばいいのか」と迷ったことはないだろうか。「良さそう」で選ぶと、不要なパターンまで導入してしまったり、逆に本当に必要なものを見落としたりしがちだ。

順序はむしろ逆がいい。**自システムの文脈（フォース）を見積もり、二者択一を解き、パターンの程度を合わせる**ことで、必要十分な構成が自然と導かれる。
...
[駆動変数（フォース）F1–F9](../foundations/forces.md) を、自システムの文脈に照らして「高/中/低」で評価する。フォースがパターン選定の判断の基盤になる。
...
二者択一は独立ではなく、互いに連動する。たとえば非同期を選ぶとタイムアウトの自由度が上がり、マルチエージェントを選ぶと予算管理が複雑になる。→ [ダイヤル×二者択一の相互作用](interactions.md)
...
ただし、目安値は**出発点**に過ぎない。本番のメトリクスを見ながら継続的に調整していこう。
...
選定理由——どのフォースが効いて、どの二者択一をどう解き、どのダイヤルをいくつに設定したか——を [意思決定記録（ADR）](adr-template.md) に残す。根拠を残さないと、問題が起きたときに何を調整すればいいかわからなくなる。

### ⑥ 見直す

フォースは固定値ではない。機能追加やユーザー層の変化、規制改正でフォースは変わりうる。変わったら①に戻って再評価する。この見直しの仕組みは [#53 Agent Change Management](../patterns/12-governance/53-agent-change-management.md) で規律化できる。
