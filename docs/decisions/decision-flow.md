...
AIエージェントのアーキテクチャを設計するとき、59個のパターンカタログから「良さそうなもの」を選ぶアプローチでは、過剰適用か適用不足になりがちである。正しい順序はむしろ逆で、**自システムの文脈（フォース）を見積もり、設計上の二者択一を解き、パターンの程度を合わせる**ことによって、必要十分な構成が導かれる。
...
個別パターンを層として重ね、複合構成を組み立てる。[リファレンスアーキテクチャ](../reference-architectures.md) の「選定の5問」に答えることで、適切な構成が導かれる。
