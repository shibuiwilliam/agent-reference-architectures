...
[程度（ダイヤル）](tuning-dials.md) と [相反（二者択一）](tradeoffs.md) は別々のカタログに整理されているが、実際の設計では**連動**する。ある二者択一を解くと、関連するダイヤルの値域が変わり、別の二者択一の前提も変わる。この連鎖を意識しないと、個別には合理的でも全体として矛盾した設計になる。
