...
多くのパターンには「どの程度適用するか」を決めるパラメータがある。タイムアウト値、リトライ回数、ガードレールの厳しさなどがそれにあたる。小さすぎても大きすぎても問題が起き、最適点はシステムの文脈——すなわち [駆動変数（フォース）](../foundations/forces.md) の値域——で決まる。

このページは主要なダイヤルを一覧し、目安値と調整の決め手を示す。目安値はあくまで**出発点**であり、本番データで検証して調整すること。
...
ダイヤルの値は「一度決めたら終わり」ではない。[#53 Agent Change Management](../patterns/12-governance/53-agent-change-management.md) のプロセスで定期的に見直す。
