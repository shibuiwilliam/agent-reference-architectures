...
あるパターンを選ぶと、別のパターンが排他的になったり、トレードオフが生じたりする場面がある。どちらが正しいかは文脈——つまり [駆動変数](../foundations/forces.md) の値域——によって決まる。このページでは主要な二者択一を一覧し、判断の入り口を示す。
...
相反の選定は一度きりの判断ではない。フォースの値域が変わったとき——たとえば規制強化やスケール増大などがあった場合——には再評価が必要である。→ [パターンのパラメータ化](parameterization.md)
