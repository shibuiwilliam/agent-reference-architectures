...
理論だけでは「結局どう使うのか」がわかりにくいです。ここでは [意思決定の進め方](decision-flow.md) の①〜⑤を、具体的なシステムを題材にして実演します。「フォースが二者択一とダイヤルを経て、パターン選定を駆動する」という因果の流れを、矢印で追える形にしました。
...
社内のドキュメント（Confluence/Notion）を検索し、社員の質問に回答するチャットボットです。社内ユーザー約500人、月間数千クエリです。
...
因果は常に **フォース → 決定 → パターン** の向きで流れます。この流れを [ADR](adr-template.md) に記録し、フォースが変わったら再評価します。
