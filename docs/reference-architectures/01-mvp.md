...
新しいプロダクトのアイデアを検証したい。社内の業務改善ツールとして、まず小さなチームで試したい。そんな場面で最初に必要になるのがこの最小構成です。

特徴的なのは、ガードレール・セキュリティ・コスト最適化といった「本番では必要だけれど初期には過剰な層」を意図的に省いている点です。`[F2]` 失敗コストが低く、`[F8]` 説明責任も求められない状況——たとえば社内ツールやハッカソンのプロトタイプ——で使います。

ただし「最小」は「雑」とは違います。非同期化・中断再開・構造化出力・最低限のトレースという4層は、後から層を追加するときの土台になります。この土台がないと、本番化のタイミングで全面作り直しになりがちです。
...
クライアントからのリクエストに対してすぐにジョブIDで応答し、LLM処理はバックグラウンドで実行します。この層がないと、LLMの推論時間がHTTPタイムアウトに引っかかってフロントエンドにエラーが返ります。リトライが走れば同じリクエストが多重実行されるリスクもあります。
...
全てのLLM呼び出し・ツール実行をトレースとして記録します。MVPの段階であっても、トレースがあるとデバッグの速度が桁違いに上がります。この層がないと、不具合の原因調査がLLMへの再質問の繰り返しになり、非常に非効率です。
...
社内のナレッジベースに対するQ&Aチャットボットを作るケースを考えてみましょう。ユーザーは社内のエンジニア20名程度で、社内Wikiの内容をRAGで検索して回答を返します。

クライアント（Slackボット）がユーザーの質問を受け取ると、Request-to-Job Gatewayにリクエストを投げます。Gatewayはジョブを非同期キューに入れ、Slackには「回答を生成中です」と即座に返します。バックグラウンドのワーカーがDurable Sessionとして質問を処理し、RAG検索→LLM推論を実行。出力はStructured Output Contractに従ってJSON形式で構造化され、Slackのメッセージブロックとして整形されます。すべてのLLM呼び出しはAgent Traceに記録されるので、「なぜこの回答になったか」を後から確認できます。

この構成なら、1〜2日で動くプロトタイプを立ち上げられるでしょう。ユーザーからフィードバックを集めながら、必要に応じて層を追加していきます。
