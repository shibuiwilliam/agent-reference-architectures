...
LLMは柔軟だが非決定論的であり、同じ入力に同じ出力を保証できない。決定論コア・確率論エッジは、ビジネスルール・状態遷移・金額計算・権限判定など「間違ってはならない」処理を従来のコードで書き、LLMは自然言語の解釈、非構造データの分類、ユーザー向け文章の生成など「柔軟さが必要な周辺」に限定する構成である。
...
`[F2]` 失敗コストが高い処理（金融取引・医療判断・法的手続き）を LLM の確率的出力に委ねるリスクを排除する。`[F8]` 説明責任・規制が求める「なぜその結果になったか」を、決定論コアのコードパスとして説明できる。LLM 部分で障害が起きてもコアロジックは影響を受けず、障害の影響半径を限定できる。

## 向き / 不向き

- **向き**: 金融計算、保険査定、医療プロトコル、法的文書処理など、正確性と監査可能性が必須の業務。既存の業務システムにエージェント機能を追加する場面。
- **不向き**: 探索的なリサーチやクリエイティブ生成など、タスク全体が確率的推論を本質とする場合。ルール自体が曖昧で事前にコード化できない領域。
