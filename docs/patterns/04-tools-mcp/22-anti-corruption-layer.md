...
エージェントが既存システム（ERP、CRM、レガシー API など）と直接やり取りすると、外部の命名規則やデータ形式、業務制約がエージェントの内部表現に漏れ出してしまう。Anti-Corruption Layer（ACL）はドメイン駆動設計に由来するパターンで、外部モデルとエージェント内部モデルの間に翻訳層を挟む。エージェント側は自分のドメイン語彙だけを扱い、ACL が双方向の変換を担う。
