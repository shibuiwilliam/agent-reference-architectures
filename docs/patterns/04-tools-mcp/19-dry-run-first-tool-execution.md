...
エージェントがツールを呼ぶとき、いきなり本実行してしまうと、誤った判断がそのまま副作用として反映されてしまう。本パターンでは、副作用を持つ操作を2つのフェーズに分ける。第1フェーズ（ドライラン）では実際の変更を行わず、「何が起きるか」を算出して提示する。第2フェーズ（本実行）は、ユーザーまたは自動承認ポリシーによる承認を得てから実行する。
