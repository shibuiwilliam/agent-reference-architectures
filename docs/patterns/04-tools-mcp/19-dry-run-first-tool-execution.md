...
<!-- BEGIN:GEN:meta -->
<details markdown="1">
<summary>メタデータ（機械可読） — #19 Dry-Run First Tool Execution｜ドライラン優先実行</summary>
...
| **二者択一** | — |
| **関連パターン** | #4, #31, #15 |
| **向き** | データ変更（削除・API書き込み）、インフラ変更（Terraform的操作） |
| **不向き** | 読み取り専用; 失敗コスト低; リアルタイム速度重視 |
| **要素技術** | --dry-run フラグ, DB transaction→result→rollback, API dryRun パラメータ, diff-view UI |
...
エージェントが「本番データベースから1万件のレコードを削除」と判断したとき、その操作がいきなり実行されたらどうなるか。誤った判断であれば、取り返しのつかない結果がそのまま反映されてしまう。

本パターンでは、副作用を持つ操作を2つのフェーズに分ける。第1フェーズ（ドライラン）では実際の変更を行わず、「何が起きるか」を算出して提示する。第2フェーズ（本実行）は、ユーザーまたは自動承認ポリシーによる承認を得てから実行する。
...
`[F1]` 不可逆な操作（削除、送金、公開など）をエージェントが直接実行してしまうと、やり直しがきかない。`[F2]` 失敗コストが高い操作ほど、「実行前に何が起きるかを見せる」というバッファの価値が大きくなる。また、ドライランの結果は人間承認（[#31](../06-reliability/31-human-approval-checkpoint.md)）の判断材料としても活用できる。

## 向き / 不向き

- **向き**: データ変更・API呼び出し・インフラ操作など、副作用がある操作全般に適している。Terraform plan や SQL の `EXPLAIN` と同じ思想である。
- **不向き**: 読み取り専用の操作には不要である。また、リアルタイム性が最優先でプレビューを挟む余裕がない場面や、模擬実行のコストが本実行と同等になってしまう場合にも向かない。
