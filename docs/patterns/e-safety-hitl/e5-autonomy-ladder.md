...
エージェントの自律性を「ある/なし」の二値で決めず、L0（提案のみ）から L6（完全自律）までの7段階で定義します。運転免許のように実績で信頼を獲得し昇格する一方、ポリシー違反やモデル変更時には降格します。各段階が許可するツールクラスと承認要件を明示することで、安全性と自動化の恩恵を段階的に両立させます。

## 解決する問題

エージェントに副作用を伴う操作を許可する際（[F8](../../concepts/design-forces.md)）、「全自動か全手動か」の二択は現実に合いません。全自動にすれば LLM のハルシネーション（[F4](../../concepts/design-forces.md)）が不可逆な損害を引き起こしますし、全手動にすれば人間との協働（[F17](../../concepts/design-forces.md)）の効率が消失します。

このパターンが無いと、以下の問題が発生します：

- **信頼が蓄積されません**。エージェントが何千回正しく動作しても、初日と同じ承認頻度を要求し続けます。
- **新規エージェント／新モデルへの移行が怖い**。段階的に権限を広げる仕組みがないため、「全権を与えるか、与えないか」の判断を迫られます。
- **ドメイン横断の権限管理が曖昧になります**。「このエージェントにどこまで任せてよいか」が属人的な判断に留まり、監査に耐えません。

## 選定条件（When to use / When NOT）

- **使う条件**
    - エージェントが長期間にわたって運用され、操作実績が蓄積されます。
    - `[failure_cost]` が操作カテゴリによって大きく異なり、段階的な権限移譲が合理的です。
    - 組織として「エージェントの信頼度を定量的に管理したい」という要件があります（`[accountability]` が高い環境）。
    - [E1 リスクベース承認](e1-risk-based-approval.md) の静的な閾値では硬直的すぎると感じる場面があります。
- **使わない条件（＝代替に倒す）**
    - エージェントの用途がすべて読取専用で副作用がない → [C2 Read-Free / Write-Gated](../c-tools-security/c2-read-free-write-gated.md) の Read-Free 側で十分です。
    - 単発バッチで実績の蓄積が見込めない → [E1 リスクベース承認](e1-risk-based-approval.md) の静的リスク分類で対応します。
    - `[failure_cost]` が一律に低く、すべての操作が容易にロールバック可能 → 固定の自動実行ポリシーで済みます。

## 駆動変数とチューニング（程度）

自律性は [程度カタログ](../../degrees/index.md) で定義された L0〜L6 の7段階を基盤とします。
...
- **[F-15 読取専用 vs 書込可能](../../forks/index.md) → hybrid**。本パターンは F-15 のハイブリッド戦略を時間軸に沿って展開します。初期は Read-Free 側（L0〜L1）に倒し、実績に応じて Write-Gated（L4）、さらに Bounded Auto（L5）へ段階的に移行します。`[reversibility]` が低い操作カテゴリほど、上位レベルへの昇格条件を厳しくします。
...
- **モデル変更時のリセットを忘れないでください**。新モデルは挙動が異なる可能性があるため（[F9](../../concepts/design-forces.md)）、信頼度を初期レベルに戻すか、少なくとも1段階降格します。昇格実績は旧モデル版に紐づくものであり、新モデルには引き継ぎません。
- **昇格/降格の判定を LLM に任せないでください**。判定は決定論的なルールエンジンで行います。成功率やポリシー違反数は定量的に計測可能であり、LLM の主観を挟む必要はありません。
- **ドメイン別にレベルを分離してください**。「メール送信は L5 だがDB操作は L2」のように、操作カテゴリごとに独立したレベルを持たせます。ひとつのカテゴリでの高実績が、別カテゴリの信頼を保証するわけではありません。
- **冷却期間を設けてください**。短期間に連続昇格すると、十分な検証なく高権限に到達するリスクがあります。昇格間には概ね 7〜14日の冷却期間を置きます。

## 効かせる力学（forces）

- **F4（ハルシネーション）**：LLM がハルシネーションにより誤った操作を生成しても、低レベルのエージェントは実行権限を持たず被害が限定されます。実績が十分に蓄積された（＝ハルシネーション頻度が低い）エージェントだけが高い自律性を得ます。
- **F8（ツール副作用）**：副作用を伴うツールへのアクセスを段階ごとに制御します。L0〜L1 では副作用なし、L2〜L4 では承認付き、L5〜L6 で初めて自動実行を許可します。これにより、段階的にツールの被害半径を広げます。
- **F17（人間協働）**：信頼が低い段階では人間の関与を密にし、信頼が蓄積されるにつれて関与を減らします。人間は初期のガイド役から、例外時の監督者へと役割が自然に変化します。

## 関連・代替

- [B3 自律ループ](../b-orchestration/b3-agentic-loop-budget.md)：自律ループが許可する自律度の範囲を本パターンが決定します。B3 の `max_steps` や許可ツールセットは、現在の自律性レベルから導出します。
- [C2 Read-Free / Write-Gated](../c-tools-security/c2-read-free-write-gated.md)：C2 の Read-Free / Write-Gated は本パターンの L1〜L2 に相当する静的な権限分離です。本パターンはこれを動的に昇降格する仕組みに拡張します。
- [E1 リスクベース承認](e1-risk-based-approval.md)：E1 の承認閾値を固定ではなく、本パターンの現在レベルに応じて動的に調整します。L4 では全書込に承認を要求しますが、L5 に昇格すれば低リスク書込は自動化されます。
- [E2 ポリシーのコード化](e2-policy-as-code.md)：昇格・降格の条件をポリシーとしてコード化します。属人的な判断ではなく、ポリシーエンジンがレベル遷移を管理します。
- [G4 評価ハーネス](../g-observability-ops/g4-eval-harness.md)：昇格判定に必要な成功率・エラー率・品質スコアの入力源です。G4 の評価結果が昇格/降格エンジンにフィードバックされます。
