...
すべてのリクエストを最高性能（＝最高コスト）のモデルに送る必要はない。セマンティックゲートウェイはリクエストを受け取り、難易度分類・価値推定・コスト制約をもとに最適なモデルへルーティングする。たとえば、単純な質問は軽量モデルで即答し、複雑な推論は大型モデルへ送る。これにより、平均コストを大幅に下げつつ品質を維持できる。
