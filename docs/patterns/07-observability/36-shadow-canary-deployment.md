...
エージェントの変更（プロンプト・モデル・ツール）は、評価CIを通過しても本番の全トラフィックに一括適用するのはリスクが高い。本パターンではシャドーモード（本番リクエストを複製し新版で処理するが結果はユーザーに返さない）またはカナリアモード（トラフィックの一部を新版に振る）で段階的に投入し、品質・レイテンシ・コストのメトリクスを監視する。閾値を超えた劣化を検知すれば自動でロールバックする。
...
ルーターがトラフィックを旧版と新版に重み付きで振り分ける。両者のメトリクス（正答率、レイテンシp50/p99、トークンコスト、エラー率）を同一基盤で収集し、統計的に比較する。有意な劣化が検知されれば新版への振り分けを0%に戻す。問題なければ徐々に比率を上げ、最終的に100%へ切り替える。

## 解決する課題

エージェントは非決定論的であり、オフライン評価では本番の全パターンをカバーできない。一括デプロイで品質劣化が全ユーザーに波及するリスクを、段階投入で被害半径を限定する `[F9]`。プロバイダ側のモデル更新による不意の挙動変化にも、カナリアで早期検知・ロールバックが可能になる。

## 向き / 不向き

- **向き**: 本番トラフィックが十分にあり統計的比較が成立する規模、SLAが厳しいサービス、モデル切り替えやプロンプト大改修時。
- **不向き**: リクエスト数が少なく統計的有意差が出ない場合、副作用のある処理でシャドー実行するとデータ二重書きが起きる場合（シャドー時は副作用をモックする必要がある）。
