...
[#32 Agent Trace](32-agent-trace.md) で蓄積した本番トレース（入力・コンテキスト・ツール応答）を、新しいプロンプト・モデル・ツールバージョンのエージェントに再投入し、出力の差分を比較する。合成テストデータでは再現しにくい、本番特有の入力分布やエッジケースに対して変更の影響を事前に評価できるのが大きな利点である。
