...
SaaS型エージェントサービスでは、複数テナントが同一インフラを共有する。LLMのコンテキスト窓やメモリストア、ツール呼び出し先がテナント間で混在すると、プロンプトインジェクションや単純なバグによってテナントAのデータがテナントBへ漏洩するおそれがある。本パターンでは、リクエスト受付時にテナントIDを検証し、以降の実行パス全体――コンテキスト、メモリ、ツールバインディング、ログ――をテナント単位で隔離する。
