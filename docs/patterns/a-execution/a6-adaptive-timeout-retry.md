...
forces: [F1, F7, F12]
driving_variables: [latency_budget, cost_sensitivity, failure_cost, provider_trust]
forks:
  - "F-12:hybrid"
related_patterns: [A2, A3, A7, C4, G1]
alternatives: []
...
- **サーキットブレーカとの連携** — ネットワークリトライ上限を超えたら本パターンのサーキットブレーカでサーキットを開き、一定時間リクエストを遮断して下流を守ります。
- **ストリーミング時のタイムアウト** — 全体タイムアウトよりトークン間タイムアウトが優れています。最初の1トークンまでの待ち時間と、トークン間の無応答時間を分けて監視します。

## サーキットブレーカと縮退

リトライ上限を超えた場合のエスカレーション先として、サーキットブレーカと段階的縮退（graded degradation）を本パターンと統合的に設計します。`[provider_trust]` が低いほど、早期遮断と縮退ラダーの重要性が増します。

### サーキットブレーカの状態遷移

プロバイダごとに独立したサーキットブレーカを設置し、3つの状態で管理します。

- **Closed（通常）** — リクエストを通過させます。連続失敗が閾値（概ね3〜5回、`[provider_trust]` が低いほど下げる）に達すると Open に遷移します。
- **Open（遮断）** — リクエストを即座にブロックし、縮退ラダーにフォールバックします。タイマー満了（概ね15〜60秒、指数バックオフ）で Half-Open に遷移します。
- **Half-Open（試行）** — 1〜数リクエストのみ試行します。成功すれば Closed に復帰、失敗すれば再び Open に戻ります。

ブレーカはモデル単位・リージョン単位で分離します。共有ブレーカにすると、1プロバイダの障害で全プロバイダが遮断されるリスクがあります。

### 縮退ラダー（段階的フォールバック）

サーキットブレーカが Open になった場合、以下のラダーを段階的に降りていきます。

1. **Level 1: 軽量モデル / 別プロバイダ** — プロバイダ抽象化層を介してフォールバック先に切り替えます。縮退用のプロンプトテンプレートを別途用意するか、JSON Schema 等で出力を構造化してモデル差異を吸収します。
2. **Level 2: キャッシュ応答** — 類似クエリの過去の応答をセマンティックキャッシュから返します。鮮度が許容範囲内かを検証します。
3. **Level 3: 静的フォールバック** — テンプレート応答や定型文を返します。「現在処理能力が低下しています」等のメッセージを含めます。
4. **Level 4: Fail-fast** — 全段を試行しても応答できない場合、明示的にエラーを返し人間にエスカレーションします。

`[failure_cost]` が高い領域（医療・金融等）ではラダーを浅くし、品質が保証できない応答を返すリスクを避けます。

### プロバイダ抽象化層

全プロバイダが共通インターフェース（`complete(messages, **kwargs)` / `health_check()`）を実装し、ProviderRouter が優先度順にフォールバックを試みます。これにより、フォールバック先の切り替えが呼び出し元に対して透過的になります。縮退レベルをレスポンスメタデータに付与し、呼び出し元が品質期待値を調整できるようにしておきます。

### ヘルスチェックと復旧判定

Half-Open 状態での試行成功を復旧とみなしますが、1回の成功で即座に全量を流すのではなく、トラフィックを段階的に戻す（カナリア復旧）ことを推奨します。[G1 二層観測](../g-observability-ops/g1-tiered-observability.md)にブレーカの状態遷移と縮退レベルをメトリクスとして記録し、遮断頻度の推移をプロバイダの信頼性評価に活用します。

## 効かせる力学（forces）

- **F1（長時間）** — 操作クラス別タイムアウトにより、長い推論は待ちつつ短い操作のハングは早期検出します。
- **F7（不安定）** — エラー分類＋指数バックオフで一時障害を吸収し、上限超でサーキットブレーカへ移行して連鎖障害を防ぎます。サーキットブレーカがプロバイダ単位で障害を遮断し、縮退ラダーで段階的にサービスを継続します。
...
- 本パターンのサーキットブレーカ・縮退セクション — ネットワークリトライ上限超のエスカレーション先です。
...
- [ ] 本パターンのサーキットブレーカ機能との連携（リトライ上限超の挙動）を定義したか
