...
OpenAI、Anthropic、Google、オープンソースモデルはそれぞれAPIフォーマット、function calling の仕様、トークン計算、ストップシーケンスの挙動が異なる。モデルを直接呼び出すコードが散在すると、プロバイダ切り替えが全面改修になる。本パターンは呼び出し側とモデル間に互換レイヤーを挿入し、リクエスト／レスポンスの正規化、function callingスキーマの変換、トークンカウントの統一を行う。
...
LLMプロバイダの障害・値上げ・API廃止は実際に起こる `[F9]`。モデル切り替えがプロンプト以外のコード変更を伴うと、切り替え判断が遅れ、障害時の復旧時間が長くなる。互換レイヤーがあれば、設定変更だけでフォールバック先に切り替えられる。
