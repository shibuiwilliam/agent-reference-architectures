...
LLMは確信度と正確性が一致しないことがある。Evidence-First Answer は、回答生成のワークフローを「まず根拠を集め、次にそれに基づいて回答を構成する」という二段階に分離する。根拠が見つからない場合は「わからない」と明示的に返す。回答には引用（出典ドキュメント、チャンク、URL）を付与し、ユーザーや監査者が検証可能にする。
...
ハルシネーションの最も危険な形態は「もっともらしいが根拠のない回答」である。特に規制領域 `[F8]`（法務、医療、金融）では、根拠のない助言が法的責任を生む。Evidence-First は回答の生成経路を根拠取得に依存させることで、「根拠なしに答える」パスを構造的に排除する。

## 向き / 不向き

- **向き**: 法務・医療・金融の質問応答、社内ナレッジ検索、監査対応が必要なカスタマーサポート。
- **不向き**: 創作・ブレインストーミングなど正解のないタスク、根拠ソースが存在しない探索的対話。
