...
LLMは確信度と正確性が必ずしも一致しないことがある。Evidence-First Answer は、回答生成のワークフローを「まず根拠を集め、次にそれに基づいて回答を構成する」という二段階に分離するパターンである。根拠が見つからない場合は「わからない」と明示的に返す。回答には引用（出典ドキュメント、チャンク、URL）を付与し、ユーザーや監査者が検証できるようにする。
