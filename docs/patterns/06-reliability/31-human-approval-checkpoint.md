...
エージェントの自律性が高まるほど、誤った判断が不可逆な結果をもたらすリスクも増していく。Human Approval Checkpoint は、ワークフロー上の特定ポイント（送金実行、データ削除、外部公開、契約締結など）でエージェントの実行を一時停止し、人間に「承認/却下/修正」の判断を求めるパターンである。承認待ちの間、エージェントのセッション状態は永続化され、承認が得られた後に処理を再開する。
