...
- **F14（プロンプトインジェクション）**：入力サニタイズと認可をコードで強制し、インジェクションによる権限昇格を防ぎます。動的スコーピングにより、そもそも危険なツールが LLM の選択肢に入らない状態を作れます。
...
- [C5 Capability Lease](c5-capability-lease.md)：ゲートウェイが発行する短命トークンで権限の有効期間を制限します。長時間セッションでの権限拡散を防ぎます。
