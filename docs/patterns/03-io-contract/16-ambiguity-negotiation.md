...
ユーザー入力には省略や多義性、暗黙の前提が含まれることが多い。エージェントが「たぶんこうだろう」と推測して実行すると、不可逆な誤操作や期待外れの結果を招きかねない。本パターンでは、曖昧度が閾値を超えた場合に実行を保留し、選択肢の提示や確認質問によってユーザーと意図を交渉する。交渉の結果は構造化された確定意図として下流に渡す。
