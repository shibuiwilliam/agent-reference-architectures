...
<!-- BEGIN:GEN:meta -->
<details markdown="1">
<summary>メタデータ（機械可読） — #7 Streaming Progress｜進捗ストリーミング</summary>
...
| **二者択一** | push-vs-pull |
| **関連パターン** | #1, #6, #32 |
| **向き** | 5秒以上のタスク、ユーザーのインタラクティブ監視、規制トレースログ |
| **不向き** | サブ秒処理; バッチのサイレントモード |
| **要素技術** | SSE, WebSocket, gRPC streaming, Redis Streams, Kafka, JSON Lines |
...
エージェントに調査を任せたまま2分間画面が無反応だったら、ユーザーは「フリーズしたのでは」と不安になり、ブラウザをリロードしたくなる。処理が順調に進んでいたとしても、見えなければ伝わらない。

このパターンでは、ステップ完了・ツール呼び出し・中間判断などのイベントを、構造化された進捗メッセージとしてリアルタイムに配信する。単なるトークンストリーミングではなく、「何を意図し、何を実行し、何が返ったか」というセマンティックな進捗を送る点が特徴である。
