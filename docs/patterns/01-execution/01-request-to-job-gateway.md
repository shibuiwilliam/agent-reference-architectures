...
ユーザーからの1リクエストを同期レスポンスで返そうとせず、受付時に `session_id` / `job_id` を即座に発行して `202 Accepted` を返す。実処理はキューやワークフローエンジンへ渡し、ワーカー群が背後で実行する。進捗と最終結果は polling・SSE・WebSocket・Webhook で受け取る。Web層は「受付」だけを担い、エージェント実行層と独立にスケールする。
...
受付APIはジョブをキュー（SQS / Pub/Sub / Redis Stream など）へ積み、状態を `pending → running → (partial) → done / failed / timeout` で外部ストアに永続化する。中間トークンや進捗は別チャネルでストリーミングする。

## 解決する課題

長時間セッション（数秒〜数十分）がHTTPコネクションを占有し、ロードバランサやプロキシのタイムアウトで切断される問題を解消する。Web層とエージェント層を独立スケールでき、リクエストのバーストを吸収できる。失敗・再実行・中断・人間承認待ちも、ジョブの状態遷移として素直に扱えるようになる。

## 向き / 不向き

- **向き**: リサーチ・資料作成・コード生成・データ分析など、完了までに複数ステップや外部検索を伴うタスク。
- **不向き**: 数百ミリ秒〜数秒で返したいオートコンプリート・単純分類・同期トランザクション。
...
- **同期待機の閾値**（[#58 Sync Facade](58-sync-facade-over-async-core.md) と併用する場合）— 短すぎると非同期化の頻度が上がりUXが煩雑 ⇔ 長すぎるとコネクション占有 / 決め手 `[F4]` / 目安: 5〜10秒。→ [程度ダイヤル](../../decisions/tuning-dials.md)
