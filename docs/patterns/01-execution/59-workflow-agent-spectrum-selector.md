...
現実のシステムでは、「全部をワークフローで組む」か「全部をエージェントに任せる」かのどちらも最適解にならないことが多い。このパターンでは、サブタスクごとにタスクの変動性 `[F6]` と失敗コスト `[F2]` を評価し、決定論的ワークフロー・ガードレール付きエージェント・自律エージェントのいずれで実行するかを選定する。設計時の判断フレームワークとして使えるほか、実行時に動的に切り替える仕組みとしても構成できる。
