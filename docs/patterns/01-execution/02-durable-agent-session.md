...
エージェントのセッション状態――会話履歴、中間結果、ステップカウンタ、取得済みコンテキスト――をインメモリではなく外部の耐久ストアに書き出す。これにより、プロセスが落ちてもストアから状態を復元し、最後のチェックポイントから再開できるようになる。また、ステートレスなワーカーが任意のセッションをピックアップできるため、スケールアウトやローリングデプロイとも両立しやすい。
