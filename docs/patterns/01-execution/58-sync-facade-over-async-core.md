...
すべてのリクエストを非同期にすると、すぐ返せるものまでポーリングやWebSocket接続が必要になり、UXが煩雑になる。逆にすべてを同期にすると、長い処理でコネクションタイムアウトが起きてしまう。このパターンでは、同期ファサードがまず処理を開始し、閾値時間内に完了すればそのままレスポンスを返す。超えそうになった場合はバックグラウンドジョブに昇格させ、`202 Accepted + job_id` を返して非同期モードに切り替える。
