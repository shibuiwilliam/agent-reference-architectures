...
エージェントが「カレンダー予約 → チケット作成 → メール送信」のように複数の副作用を連鎖させるとき、途中ステップの失敗によって不整合が生じることがある。Sagaパターンでは、各ステップに対応する補償アクション（予約取り消し、チケットクローズ、訂正メール送信）を事前に定義しておき、失敗時に逆順で実行する。これは分散トランザクションのSagaパターンを、エージェントの文脈に適用したものである。
